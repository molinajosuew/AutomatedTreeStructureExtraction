function [D,S,Q] = perform_fast_marching(W, segmentationn, start_points,end_points,nb_iter_max, options)
...
if size(start_points,1)~=d
    error('start_points should be (d,k) dimensional with k=2 or 3.');
        ...
        L = 0.5*ones(size(W,1), size(W,2), size(W,3));
        %L((segmentationn==0))=-1e9;

L(L==-Inf)=-1e9;
L(L==Inf)=1e9;
%nb_iter_max=1.2*max(size(W))^d;
%nb_iter_max = min(nb_iter_max, 1.2*max(size(W))^d);
%end_points= [1;1;1];
    %end_pointss= find(segmentationn==0);
    %s=size(W);
    %[K, L, M]=ind2sub(s,end_pointss);
    %end_points=[K'; L'; M'];

%H=10*ones(512,512,31);
H=[];
values=[];%values=10;
%values=zeros(numel(start_points),1);
%end_points=end_points-1;
% use fast C-coded version if possible
se=size(end_points);
nb=se(2);
ss=size(start_points);
ns=ss(2);
if d==2
...
elseif d==3
    [D,S,Q] = perform_front_propagation_3d(W,start_points-ones(3,ns),end_points-ones(3,nb),nb_iter_max, H, L, values);
end
Q = Q+1;
